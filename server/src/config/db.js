...
const problemCatalog = require('./problemCatalog');
...
const seedProblems = async (dbPool) => {
  const problems = problemCatalog.map(({ id, ...problem }) => problem);
  const normalizedTitles = problems.map((problem) => problem.title.trim().toLowerCase());
  let removedCount = 0;
  let insertedCount = 0;
  let updatedCount = 0;
  ...
  try {
    const deleteResult = await dbPool.query(
      `DELETE FROM problems
       WHERE NOT (LOWER(BTRIM(title)) = ANY($1::text[]))`,
      [normalizedTitles]
    );
...
const runMigrations = async () => {
  ...
  try {
    const problemSync = await seedProblems(dbPool);
    console.log(
      `  ✅ Synced problem catalog (${problemSync.totalCount} total, ` +
      `${problemSync.insertedCount} inserted, ${problemSync.updatedCount} updated, ${problemSync.removedCount} removed)`
    );
  } catch (error) {
